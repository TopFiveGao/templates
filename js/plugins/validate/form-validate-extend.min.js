$.validator.setDefaults({
	...
	highlight: function(e) {
		$(e).closest(".row").removeClass("has-success").addClass("has-error")
	},
	success: function(e) {
		e.closest(".row").removeClass("has-error").addClass("has-success")
	},
	errorElement: "span",
...
//判断是否为MAC
$.validator.addMethod("isMac", function(value, element, arg){
  	var reg = RegExp(/^([A-Z0-9]{2})-([A-Z0-9]{2})-([A-Z0-9]{2})-([A-Z0-9]{2})-([A-Z0-9]{2})-([A-Z0-9]{2})$/);
	return this.optional(element) || reg.test(value);
 },"请输入正确的MAC地址");
 //判断是否大于
 ...
 //判断是否小于
 $.validator.addMethod("isLesser", function(value, element, param){
 	if($(param).val()==''){return true;}
 	return this.optional(element) || $(param).val()>=value;
 },"必须小于等于另一项");
 //用户名校验
 $.validator.addMethod("isUserName",function(value, element) {
		return this.optional(element) ||  /^[a-zA-Z]\w{6,20}$/ig.test(value);
	}, "只能以字母开头的包含字母数字和下划线6~20字符");
//身份证号码的验证规则
