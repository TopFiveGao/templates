...
function del(systemid) {
	parent.layer.confirm('您是否要删除该条数据？', {
		...
		saveData(api_path + "/Common/Writer.i", "aqgl_aqzzbz_delete", param,
			function() {
				refreshTable();
			});
...
function add() {
	parent.layer.open({
		...
		shade: 0.8,
		area: ['800px', '650px'],
		content: getRootPath() + '/jwxt/pages/aqgl/aqzzbz/aqzzbz_add.html?bjbh='+treeType.id,
		end: refreshTable
	});
}
//修改
function update(id) {
	parent.layer.open({
		...
		shade: 0.8,
		area: ['800px', '650px'],
		content: getRootPath() + '/jwxt/pages/aqgl/aqzzbz/aqzzbz_update.html?systemid=' + id,
		end: refreshTable
	});
}
