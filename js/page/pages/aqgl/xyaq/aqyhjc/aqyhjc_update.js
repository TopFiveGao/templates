...
$(document).ready(function() {
	$("#rehdbfsf").initDic({successHandler: function(){
		initDicCode();
	}});
...
function formDataBinding(){
	...
	getData(api_path+"/Common/Reader.i", "aqgl_aqyh_select", param, function(result) {
		...
		$("#jz").val(r.JZ);
		$("#name").text(r.JCR_V)
		$("#jcbm").trigger("chosen:updated");
		$("#fzr").trigger("chosen:updated");
		$("#ssxq").trigger("chosen:updated");
		formDatafubiao(r.SYSTEMID)
	});
}

function formDatafubiao(id){
	var param = {
		"zbid": id
	}
	getDatas(api_path+"/Common/Reader.i", "aqgl_aqyhmx_select", param, function(result) {
		let r = result.result;
...
function buttonRoopen(r){
	totalnum += 1;
	const html = `<tr>
			<td class="xh${totalnum}">${totalnum}</td>
			<td><input id="tab1${totalnum}" autocomplete="off" value="${r.AQYHD}" /></td>
			<td><input id="tab2${totalnum}" autocomplete="off" value="${r.YHWZJJBQKMS}" /></td>
			<td><input id="tab3${totalnum}" autocomplete="off" value="${r.ZGYQJCLYJ}" /></td>
			<td><input id="tab4${totalnum}" autocomplete="off" value="${r.ZGQX}" /></td>
			<td><input id="tab5${totalnum}" autocomplete="off" value="${r.ZGFZR}" /></td>
			<td><input id="tab6${totalnum}" autocomplete="off" value="${r.ZGWCQK}" /></td>
			<td><input id="tab7${totalnum}" autocomplete="off" value="${r.ZGYSR}" /></td>
			<td><input autocomplete="off" id="data-${totalnum}" value="${r.YSSJ_V}" /></td>
		</tr>`;
...
function add() {
		...
		for (let i = 1; i <= len; i++) {
			if($("#tab1"+i).val() != '' || $("#tab2"+i).val() != '' || $("#tab3"+i).val() != '' || $("#tab4"+i).val() != '' || $("#tab5"+i).val() != '' || $("#tab6"+i).val() != '' || $("#tab7"+i).val() != '' || $("#data-"+i).val() != ''){
				const obj = {};
				obj.xh = $(".xh"+i).text();
		...
		saveData(api_path + "/common/writer", "aqgl_aqyh_update", param, function(res) {
