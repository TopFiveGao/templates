...
function initTable() {
	...
	data_table.bootstrapTable({
		...
		queryParams: function(params) {
			...
			// if ($("#csrq").val() != "") {
			// 	param.csrqa = $("#csrq").val().split("~")[0].replace(/\s+/g,'');
			// 	param.csrqb = $("#csrq").val().split("~")[1].replace(/\s+/g,'');
			// };		
				if ($("#fzr").val() != "") {
					param.fzr = $("#fzr").val();
				};
			...
			if ($("#hdsj").val() != "") {
					...
					param.hdsja = $("#hdsj").val().split("~")[0].replace(/\s+/g,'');
					param.hdsjb = $("#hdsj").val().split("~")[1].replace(/\s+/g,'');
			};
...
function del(systemid) {
	parent.layer.confirm('您是否要删除该条数据？', {
		...
		saveData(api_path + "/Common/Writer.i", "aqgl_aqjy_delete", param,
			function() {
				refreshTable();
			});
...
function add() {
	parent.layer.open({
		...
		shade: 0.8,
		area: ['800px', '650px'],
		content: getRootPath() + '/jwxt/pages/aqgl/xsaqjy/xsaqjy_add.html?bjbh='+treeType.id,
		end: refreshTable
	});
}
//修改
function update(id) {
	parent.layer.open({
		...
		shade: 0.8,
		area: ['800px', '650px'],
		content: getRootPath() + '/jwxt/pages/aqgl/xsaqjy/xsaqjy_update.html?systemid=' + id,
		end: refreshTable
	});
}
